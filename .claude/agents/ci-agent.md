...
- `splunk-forwarder-operator-pull-request.yaml`: PR validation
...
# tkn pipeline validate -f .tekton/splunk-forwarder-operator-pull-request.yaml
...
```text
CI Status: FAILING
Pipeline: splunk-forwarder-operator-pull-request
Stage: golangci-lint
Error: Exit code 1

Local Reproduction:
  make go-check
  # Output shows 3 linter errors in pkg/kube/daemonset.go
